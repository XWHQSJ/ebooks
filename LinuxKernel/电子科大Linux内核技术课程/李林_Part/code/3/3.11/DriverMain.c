...
static int DriverInitialize(void)
{
	...
	for(i = 0; i < MAX_NUMNODES; i++)
	{
		...
		DEBUG_PRINT(DEVICE_NAME " node_area[%d] = 0x%lx\n", i, (unsigned long)pNode);
		
		DEBUG_PRINT(DEVICE_NAME " zonelist[0]:\n");
		MEM_PRINT((unsigned long)(pNode->node_zonelists[0]._zonerefs), 4 * sizeof(struct zoneref));

		DEBUG_PRINT(DEVICE_NAME " zonelist[1]:\n");
		MEM_PRINT((unsigned long)(pNode->node_zonelists[1]._zonerefs), 4 * sizeof(struct zoneref));
	}
