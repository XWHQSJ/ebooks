...
static int DriverInitialize(void)
{
	...
	pPage = pfn_to_page(0x3fee0);

	DEBUG_PRINT(DEVICE_NAME " page->flag : %lx\n", pPage->flags);
	DEBUG_PRINT(DEVICE_NAME " page->_count : %lx\n", (unsigned long)pPage->_count.counter);
	DEBUG_PRINT(DEVICE_NAME " page->lru.next : %lx\n", (unsigned long)pPage->lru.next);
	DEBUG_PRINT(DEVICE_NAME " page->lru.prev : %lx\n", (unsigned long)pPage->lru.prev);
