...
void PrintPageTable(unsigned long addr)
{
	pgd_t *pgde;
	pud_t *pude;
	pmd_t *pmde;
	...
	pgde = pgd_offset(current->mm, addr);
	DEBUG_PRINT(DEVICE_NAME " pgde = 0x%lx\n", pgde->pgd);

	pude = pud_offset(pgde, addr);
	DEBUG_PRINT(DEVICE_NAME " pude = 0x%lx\n", pude->pud);

	pmde = pmd_offset(pude, addr);
	DEBUG_PRINT(DEVICE_NAME " pmde = 0x%lx\n", pmde->pmd);

	pte = pte_offset_kernel(pmde, addr);
	DEBUG_PRINT(DEVICE_NAME " pte = 0x%lx\n", pte->pte);
}
...
static int DriverInitialize(void)
{
	...
	addr = ioremap_cache(0x3c000000, 64*1024*1024);
	DEBUG_PRINT(DEVICE_NAME " addr = 0x%lx\n", (unsigned long)addr);

	*((unsigned long *)addr) = 0x12345678;
	DEBUG_PRINT(DEVICE_NAME " *addr = 0x%lx\n", *((unsigned long *)addr));

	PrintPageTable((unsigned long)addr);
