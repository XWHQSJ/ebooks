...
static int DriverInitialize(void)
{
	...
	DEBUG_PRINT(DEVICE_NAME " node_states N_POSSIBLE = 0x%lx\n", node_states[N_POSSIBLE].bits[0]);
	DEBUG_PRINT(DEVICE_NAME " node_states N_ONLINE = 0x%lx\n", node_states[N_ONLINE].bits[0]);
	DEBUG_PRINT(DEVICE_NAME " node_states N_NORMAL_MEMORY = 0x%lx\n", node_states[N_NORMAL_MEMORY].bits[0]);
	DEBUG_PRINT(DEVICE_NAME " node_states N_MEMORY = 0x%lx\n", node_states[N_MEMORY].bits[0]);
	DEBUG_PRINT(DEVICE_NAME " node_states N_CPU = 0x%lx\n", node_states[N_CPU].bits[0]);
