...
static int DriverInitialize(void)
{
	...
	DEBUG_PRINT(DEVICE_NAME " __PAGE_OFFSET = 0x%lx\n", __PAGE_OFFSET);

	DEBUG_PRINT(DEVICE_NAME " VMALLOC_START = 0x%lx\n", VMALLOC_START);
	DEBUG_PRINT(DEVICE_NAME " VMALLOC_END = 0x%lx\n", VMALLOC_END);
	
	DEBUG_PRINT(DEVICE_NAME " VMEMMAP_START = 0x%lx\n", VMEMMAP_START);
	
	DEBUG_PRINT(DEVICE_NAME " __START_KERNEL_map = 0x%lx\n", __START_KERNEL_map);
	DEBUG_PRINT(DEVICE_NAME " KERNEL_IMAGE_SIZE = 0x%lx\n", (unsigned long)KERNEL_IMAGE_SIZE);

	DEBUG_PRINT(DEVICE_NAME " MODULES_VADDR = 0x%lx\n", MODULES_VADDR);
	DEBUG_PRINT(DEVICE_NAME " MODULES_END = 0x%lx\n", MODULES_END);
