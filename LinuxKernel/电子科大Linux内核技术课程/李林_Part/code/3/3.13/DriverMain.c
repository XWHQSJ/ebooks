...
void DisplayPageInfo(struct page *pPage)
{
	DEBUG_PRINT(DEVICE_NAME " First Page:\n");
	MEM_PRINT((unsigned long)pPage, sizeof(struct page));

	DEBUG_PRINT(DEVICE_NAME " Second Page:\n");
	MEM_PRINT((unsigned long)(pPage + 1), sizeof(struct page));

	__free_pages(pPage, 1);

	DEBUG_PRINT(DEVICE_NAME " After free:\n");
...
static int DriverInitialize(void)
{
	...
	pPage = alloc_pages(GFP_KERNEL | __GFP_DMA, 1);
	DisplayPageInfo(pPage);
	
	DEBUG_PRINT(DEVICE_NAME "\n");

	pPage = alloc_pages(GFP_KERNEL, 1);
	DisplayPageInfo(pPage);
